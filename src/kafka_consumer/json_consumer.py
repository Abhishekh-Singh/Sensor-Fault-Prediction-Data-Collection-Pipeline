...
def consumer_using_sample_file(topic,file_path):
    ...
    while True:
        try:
            ...
            if msg is None:
                continue
## message ddeserializes to dictionaries and then to objects
            record: Generic = json_deserializer(msg.value(), SerializationContext(msg.topic(), MessageField.VALUE))   ## converts data to objects in record 
